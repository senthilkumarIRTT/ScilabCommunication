function [y] = gfsub(A,B,varargin)
    //GFSUB Subtracts a ploynomial from the other over a Galois field.
    
    //GFSUB(A,B) subtracts B from A over GF(2)
    //If one of the two inputs is shorter than the other, than zero padding is done to the shorter array to
    //make the two elements of equal length.
    
    //GFSUb(A,B,P) subtracts B from A over GF(P).
    //P has to be prime.
    ...
    //Adjust the length depending on LENGTH parameter
    if  n1<len then
            y(:,n1+1:len)=0
    else
           y=y(:,1:len);
