function [y] = helscanintrlv(x, nrow, ncol, hstep)
//Y = HELSCANINTRLV(X, NROW, NCOL, HSTEP) 

//Permutes symbols by filling a matrix by rows and reading them out 
//by scanning along the diagonals of this array.
//X is the data that will be filled row by row.
//Y will be the output read along the diagonal.
...
if ((nrow*ncol) ~= size(x,1))            
    error("The product of NROW and NCOL must equal the length of the sequence in X.")
end;
